...
# 1. Read all the folder names of subjects/persons provided in training data folder. In this projects we have folder names: `s1','s2`.
...
# I am using OpenCV's **LBP face detector**. On _line 66_, I convert the image to grayscale because most operations in OpenCV are performed in gray scale, then on _line 70_ I load LBP face detector using `cv2.CascadeClassifier` class. After that on _line 74_ I use `cv2.CascadeClassifier` class' `detectMultiScale` method to detect all the faces in the image. on _line 82_, from detected faces I only pick the first face because in one image there will be only one face (under the assumption that there will be only one prominent face). As faces returned by `detectMultiScale` method are actually rectangles (x, y, width, height) and not actual faces images so we have to extract face image area from the main image. So on _line 85_ I extract face area from gray image and return both the face image area and face rectangle.
# Now we have got a face detector and we know the 4 steps to prepare the data, so we are ready to code the prepare data step? Yes? So let's do it.
...
# **(step-1)** On _line 97_ I am using `os.listdir` method to read names of all folders stored on path passed to function as parameter. On _line 99-102_ I am defining labels and faces vectors.
# **(step-2)** After that I traverse through all subjects' folder names and from each subject's folder name on _line 116_ I am extracting the label information. As folder names follow the `sLabel` naming convention so removing the  letter `s` from folder name will give us the label assigned to that subject.
# **(step-3)** On _line 123_, I read all the images names of the current subject being traversed and on _line 128-155_ I traverse those images one by one. On _line 142-143_ I am using OpenCV's `imshow(window_title, image)` along with OpenCV's `waitKey(interval)` method to display the current image being traveresed. The `waitKey(interval)` method pauses the code flow for the given interval (milliseconds), I am using it with 50ms interval so that we can view the image window for 100ms. On _line 146_, I detect face from the current image being traversed.
# **(step-4)** On _line 151-155_, I add the detected face and label to their respective vectors.
...
# Let's call this function on images of the beautiful celebrities to prepare data for training of our Face Recognizer. Below is a simple code to do that.
...
#It's time to train our own face recognizer so that once trained it can recognize new faces of the persons it was trained on.
...
# I am going to use LBPH face recognizer.
...
# Now that we have the prediction function well defined, next step is to actually call this function on our test images and display those test images to see if our face recognizer correctly recognized them. So let's do it.
